...
class AppController extends Controller
{
    ...
    public function app(Request $request)
    {
        if($this->storage_space != "same_server"){
            ...
            Log::info('User url samz', ['url' => $url_aws]);
        } else{
            $url_aws = url('/').'';
            Log::info('User url', ['url' => $url_aws]);
        }

        $server_keys = DB::table('fcm')->first();
        ...
        $refertext = "Parrainez et gagnez le montant du portefeuille jusqu'à partir de ".$min." to ".$max;
    ...
    public function trackorder(Request $request)
    {
        ...
        if($order){
            ...
            $ongoings=DB::table('address')->where('address_id',$order->address_id)->first();
