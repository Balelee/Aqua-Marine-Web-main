...
class CartController extends Controller
{
        ...
        if($cart_items){
            ...
            if ($store_id != $store_id){
                ...
                if($deal){
                    ...
                    if ($p) {
                        // La requête a retourné un résultat, on peut accéder à la propriété "price"
                        $price = round($p->price, 0);
                        // Faites ce que vous avez besoin de faire avec $price ici
                    } else {
                        // La requête n'a pas retourné de résultat, traitez ce cas d'erreur si nécessaire
                        $message = array('status'=>'014', 'message'=>'sp est null', 'data'=>$p );
                        return $message;
                    }
                }
    ...
    public function make_an_order(Request $request)
    {
        ...
        $ord_get = DB::table('orders')->where('payment_method', NULL)->where('user_id', $user_id)->get();

        foreach($ord_get as $ord_s){

            DB::table('store_orders')->where('order_cart_id', $ord_s->cart_id)->delete();
        }

        $ordelete = DB::table('orders')->where('payment_method', NULL)->where('user_id', $user_id)->delete();

        $store = DB::table('store_orders')->where('store_approval', $user_id)->where('order_cart_id',"incart")->first();
        ...
        $tax_price=0;
        $pricecheck=0;
        $ph = DB::table('users')->select('user_phone','wallet')->where('id',$user_id)->first();
        $user_phone = $ph->user_phone;
        ...
        $delcharge=DB::table('freedeliverycart')->where('store_id', $store_id)->first();

        Log::info('Store cart', ['store_id' => $store_id]);
