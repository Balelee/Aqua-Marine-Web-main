...
class OrderController extends Controller
{
    ...
    public function checkout(Request $request)
    {
        ...
        if($request->payment_gateway == "" || $request->payment_method == "Wallet" || $request->payment_method == "wallet" || $request->payment_method == "WALLET"  ){
        ...
        $orderr = DB::table('orders')->where('cart_id', $cart_id)->first();

        $cart_id=$request->cart_id;

        $cart =  DB::table('orders')->where('cart_id', $cart_id)->first();

        $store_id = $cart->store_id;

        $getD = DB::table('store')->where('id', $store_id)->first();
        ...
        $ph = DB::table('users')->select('name','user_phone','wallet')->where('id',$user_id)->first();
        ...
        if ($payment_method == 'COD' || $payment_method =='cod'){
            ...
            /////send mail
            $email = DB::table('notificationby')->select('email','app')->where('user_id',$user_id)->first();

            $q = DB::table('users')->select('email','name','device_id')->where('id',$user_id)->first();
            ...
            if($email_status == 1){
                $codorderplaced =
                    $this->codorderplacedMail($cart_id,$prod_name,$price2,$delivery_date,$time_slot,$user_email,$user_name);
            }
            ...
            if($email->app ==1){
                ...
                $codorderplaced =
                    $this->codorderplacedinapp($cart_id,$prod_name,$price2,$delivery_date,$time_slot,$user_email,$user_name,$user_id,$device_id);

            }

            $orderr1 = DB::table('orders')->where('cart_id', $cart_id)->first();
            ...
            if($getD){
                ...
                $orderplacedmsgstore =
                    $this->ordersuccessfullstore($cart_id,$prod_name,$price2,$delivery_date,$time_slot,$store_phone);
                $codorderplacedstore =
                    $this->codorderplacedMailstore($cart_id,$prod_name,$price2,$delivery_date,$time_slot,$user_email,$user_name, $store_n,$store_email);

                $codorderplacedstore =
                    $this->codorderplacedinappstore($cart_id,$prod_name,$price2,$delivery_date,$time_slot,$user_email,$user_name,$store_n,$store_id);
            }
            ...
            $codorderplacedadmin =
                $this->codorderplacedMailadmin($cart_id,$prod_name,$price2,$delivery_date,$time_slot,$user_email,$user_name,$store_n,$admin_email,$admin_name);


            $delete = DB::table('store_orders')->where('store_approval',$user_id)->where('order_cart_id', 'incart')
                ->delete();


            $cart_status=DB::table('cart_status')->insert(['pending'=>Carbon::now(), 'cart_id'=>$cart_id]);
            ...
            if($payment_status=='success'){
                ...
                if($sms_status == 1){
                    ...
                    /////send sms/////
                    $codorderplaced =
                        $this->ordersuccessfull($cart_id,$prod_name,$price2,$delivery_date,$time_slot,$user_phone);
                }
                /////send mail
                ...
                if($email_status == 1){
                    ...
                    ///sending mails//
                    $orderplaced =
                        $this->orderplacedMail($cart_id,$prod_name,$price2,$delivery_date,$time_slot,$user_email,$user_name);

                }
                if($email->app == 1){
                    ///////send notification to User//////
                    $codorderplaced =
                        $this->codorderplacedinapp($cart_id,$prod_name,$price2,$delivery_date,$time_slot,$user_email,$user_name,$user_id);
    ...
    public function ongoing(Request $request)
    {
        ...
        if(count($ongoing)>0){
            ...
            foreach($ongoing as $ongoings){
                ...
                $order = DB::table('store_orders')->where('order_cart_id',$ongoings->cart_id)->get();
                $ongoings->discountonmrp = $ongoings->total_products_mrp - $ongoings->price_without_delivery;

                Log::info('New unverified user created', ['user_id' => $ongoings->total_products_mrp]);
                Log::info('New unverified user created', ['user_id' => $ongoings->price_without_delivery]);

                $ongoings->total_items=count($order);

                foreach($order as $orderssss){

                    $checkstore= DB::Table('orders')->where('cart_id',$ongoings->cart_id)->first();
