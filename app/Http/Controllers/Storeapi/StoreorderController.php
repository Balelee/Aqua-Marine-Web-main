...
class StoreorderController extends Controller
{
    ...
    public function nextdayorders(Request $request)
    {
        ...
        $ord =DB::table('orders')
            ...
            ->select('orders.cart_id','orders.paid_by_wallet','orders.coupon_discount', 'delivery_boy.dboy_id', 'orders.delivery_charge','users.name', 'users.user_phone', 'orders.delivery_date', 'orders.total_price','orders.delivery_charge','orders.rem_price','orders.payment_status','delivery_boy.boy_name','delivery_boy.boy_phone','orders.time_slot','orders.order_status','orders.payment_method','users.user_phone','address.*')
        ...
        if(count($ord)>0){
            foreach($ord as $ords){
                ...
                $data[]=array('paid_by_wallet'=>$ords->paid_by_wallet,'coupon_discount'=>$ords->coupon_discount,'delivery_charge'=>$ords->delivery_charge,'user_address'=>$ords->house_no.','.$ords->society.','.$ords->city.','.$ords->landmark.','.$ords->state.','.$ords->pincode, 'cart_id'=>$cart_id,'user_name'=>$ords->name, 'user_phone'=>$ords->user_phone,
                    'remaining_price'=>$ords->rem_price,'order_price'=>$ords->total_price,'delivery_boy_name'=>$ords->boy_name,'delivery_boy_phone'=>$ords->boy_phone,'delivery_boy_id' => $order->dboy_id, 'delivery_date'=>$ords->delivery_date,'time_slot'=>$ords->time_slot,'payment_mode'=>$ords->payment_method,  'payment_status'=>$ords->payment_status,'order_status'=>$ords->order_status, 'customer_phone'    =>$ords->user_phone,'order_details'=>$details);
            }
        }
        else{
...
public function pastorders(Request $request)
{
    ...
    $store_id = $request->store_id;
    $status = $request->my_status;
    $query = $request->my_query;

    // Vérifie l'existence du magasin
    $store = DB::table('store')->where('id', $store_id)->first();
    if (!$store) {
        return response()->json(['error' => 'Store not found'], 404);
    }

    // Commandes avec une date de livraison antérieure à aujourd'hui
    $orders = DB::table('orders')
        ...
        ->select(
            ...
            'orders.order_status',
            'orders.time_slot',
            'delivery_boy.boy_name',
            'delivery_boy.boy_phone',
    ...
    if ($query) {
        $orders->where(function($q) use ($query) {
            $q->where('users.name', 'like', "%{$query}%")
              ->orWhere('users.user_phone', 'like', "%{$query}%");
        });
    }

    $orders = $orders->orderByDesc('orders.delivery_date')
                     ->paginate(10);
    ...
    if ($orders->count()) {
        foreach ($orders as $order) {
            ...
            $data[] = [
                ...
                'remaining_price'   => $order->rem_price,
                'delivery_boy_name' => $order->boy_name,
                'delivery_boy_id'   => $order->dboy_id, 
                'delivery_boy_phone'=> $order->boy_phone,
        ...
        $data[] = ['order_details' => 'no past orders found'];
    }
