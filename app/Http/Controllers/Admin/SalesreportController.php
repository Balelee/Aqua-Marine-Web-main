...
class SalesreportController extends Controller
{
    ...
    public function orders(Request $request)
    {
        ...
        $date = $request->sel_date ?? Carbon::today()->format('d-m-Y');
        $to_date = $request->to_date ?? Carbon::today()->format('d-m-Y');
        $next_date = date('d F Y', strtotime($date));
        $next_date2 = date('d F Y', strtotime($to_date));
        $title = $payment_method." Commandes du ".$next_date." - ".$next_date2;
        $admin_email=Auth::guard('admin')->user()->email;
