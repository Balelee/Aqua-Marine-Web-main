...
class DeliveryController extends Controller
{
     ...
     public function AddNewD_boy(Request $request)
    {
        ...
        if ($response->status === "OK" && !empty($response->results)) {
            // Utilisez $lat et $lng ici pour la suite de votre code
            $lat = $response->results[0]->geometry->location->lat;
            $lng = $response->results[0]->geometry->location->lng;
        } else {
            // Gestion des erreurs si la réponse est invalide ou vide
            $message = array('status'=>'00', 'message'=>'erreurs lat et lnt invalide', 'response'=>$response);
            return $message;
        }
        }
        else{
