...
class StoreController extends Controller
{
    ...
    public function storeadd(Request $request)
    {
        ...
        $date=date('d-m-Y');
        $checkmap = DB::table('map_api')->first();
        $mapset= DB::table('map_settings')->first();
        $checkgeo = DB::table('geocoding_key')->first();
        ...
        $response =json_decode(file_get_contents("https://maps.googleapis.com/maps/api/geocode/json?address=".urlencode($address1)."&key=".$checkgeo->geoKey));
    ...
    public function storeupdate(Request $request)
    {
        ...
        return redirect()->back()->withSuccess(trans($mapset->google_map));
        if($mapset->mapbox == 0 && $mapset->google_map == 1){ 
