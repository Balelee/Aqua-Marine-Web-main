...
use App\Http\Controllers\ApiStoryController;
...
Route::post('api/stories', [StoryController::class, 'store']);

Route::delete('api/delete-story',[ StoryController::class, 'delete']);

Route::get('api/story/user', [StoryController::class, 'getUserStories']);

Route::get('verifyLicense', [InstallController::class,'verifyLicense'])->name('verifyLicense');

Route::get('privacy-policy', [WebHomeController::class,'privacy'])->name('privacy-policy');
...
Route::namespace("Admin")->prefix('')->group(function () {
  ...
  ///////sub category////////
        ...
        Route::get('privacys', [PagesController::class,'privacy_policy'])->name('privacys');
        Route::post('privacy/update', [PagesController::class,'updateprivacy'])->name('updateprivacy');
        ...
        Route::post('story/create', [StoryController::class,'store'])->name('adminStoryStore');
        Route::get('story/send', [StoryController::class,'adminStorySend'])->name('adminStorySend');
        Route::get('story/list', [StoryController::class,'adminStoryList'])->name('adminStoryList');
        Route::get('/stories/edit/{id}', [StoryController::class, 'edit_story'])->name('edit_story');
        Route::get('/stories/delete/{id}', [StoryController::class, 'delete_story'])->name('delete_story');
        Route::get('/stories/delete-all', [StoryController::class, 'delete_all_stories'])->name('delete_all_stories');
...
Route::post('send-notify-aqua', function (Request $request) {
    ...
    if ($credentialsFilePath && Storage::disk('public')->exists($credentialsFilePath)) {
        $credentialsContent = Storage::disk('public')->get($credentialsFilePath);
    } else {
        return response()->json(['message' => 'Fichier de configuration introuvable.'], 500);
    }

    $credentials = json_decode($credentialsContent, true);
    if (json_last_error() !== JSON_ERROR_NONE) {
        return response()->json(['message' => 'Erreur de décodage JSON : ' . json_last_error_msg()], 500);
    }
    ...
    try {
        ...
        if (!isset($token['access_token'])) {
            return response()->json(['message' => 'Impossible de récupérer le jeton d\'accès'], 500);
        }
        ...
        $fcmUrl = 'https://fcm.googleapis.com/v1/projects/aqua-marine-5fc33/messages:send';

        foreach ($tokens as $targetToken) {
            $singleMessage = json_decode($payload, true);
            $singleMessage['message']['token'] = $targetToken;
            $ch = curl_init();
            ...
            curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($singleMessage));
            $response = curl_exec($ch);
            $err = curl_error($ch);
            curl_close($ch);

            if ($err) {
                Log::error('Erreur CURL : ' . $err);
                return response()->json(['message' => 'Erreur CURL : ' . $err], 500);
            }

            Log::info('Notification envoyée : ' . $response);
        }
...
})->name('send-notify-aqua');

Route::post('upload-image', [FirebaseStorageController::class, 'upload']);
